dataframe2htmltable<-function(x, file, htmlclass="", htmlid="", htmlhead="<head><title>Table</title></head>", doctypeline="<!DOCTYPE html>", stylestring="", tableonly=F, useth=F){
	...
	# stylestring: can be used to include inline css style; no style added by default, use for example stylestring="border: 1px solid black"
	...
	for(i in 1:nrow(x)){
			cat(paste("\n<tr>", paste("<td>", x[i,], "</td>", sep="", collapse=""), "</tr>", sep=""), file=file, append=T)
	}
